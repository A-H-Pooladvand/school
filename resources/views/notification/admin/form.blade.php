    ...
    <form action="{{ $form['action'] }}" method="post" id="form">
        ...
        <div class="row">
            ...
            <div class="col-sm-4">
                ...
                <div class="form-group">
                    <img id="image-preview" class="thumbnail {{ !empty($notification->image) ? '' : 'hidden' }}" width="100%" src="{{ image_url($notification->image ?? '', 37,23,true) }}">
                </div>
                ...
                @component('_components.date_picker')
                    ...
                    @slot('value'){{ jdate($notification->publish_at ?? null)->format('Y/m/d H:i:s') }}@endslot
                ...
                @component('_components.date_picker')
                    ...
                    @slot('value'){{ !empty($notification->expire_at) ? jdate($notification->expire_at)->format('Y/m/d H:i:s') : null }}@endslot
                ...
                <div class="form-group">
                    ...
                    @component('_components.bootstrap-select--single')
                        ...
                        @slot('options')
                            <option {{ !empty($notification->status) && $notification->status === 'publish' ? 'selected' : '' }} value="publish">انتشار</option>
                            <option {{ !empty($notification->status) && $notification->status === 'draft' ? 'selected' : '' }} value="draft">پیشنویس</option>
                        @endslot
        ...
        {{--@component('_components.filemanager--multiple')
            ...
            @if(!empty($notification->galleries))
                @slot('data', $notification->galleries)
            @endif
    ...
    <div class="form-group helper-block">
        ...
        <div class="pull-left">
            @if(!empty($notification))
                {{ Breadcrumbs::render('notification-edit', $notification) }}
            @else
